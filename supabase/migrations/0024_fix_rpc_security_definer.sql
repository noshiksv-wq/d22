-- Fix search_public_dishes to be SECURITY DEFINER
-- This allows anon users to call it while it runs with owner privileges (bypasses RLS)
-- Run this in Supabase SQL Editor

-- Drop and recreate with SECURITY DEFINER
DROP FUNCTION IF EXISTS public.search_public_dishes(
...
REVOKE ALL ON FUNCTION public.search_public_dishes(
    TEXT, TEXT, DOUBLE PRECISION, DOUBLE PRECISION, 
    DOUBLE PRECISION, UUID[], JSONB
) FROM PUBLIC;
...
COMMENT ON FUNCTION public.search_public_dishes IS 'B2C public search function for finding restaurants and dishes with geospatial, dietary, and service filtering. Runs as SECURITY DEFINER to bypass RLS.';
