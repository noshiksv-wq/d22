-- Add matched_tags to search_public_dishes_by_tags_strict RPC
-- Returns ALL tags for each dish as JSONB array, not just the matched ones

-- Must DROP first because we're changing the return type signature
DROP FUNCTION IF EXISTS public.search_public_dishes_by_tags_strict(text,text,uuid[],jsonb,integer);
...
RETURNS TABLE(
  ...
  matched_tags jsonb  -- NEW: All tags for the dish as JSONB array
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
COMMENT ON FUNCTION public.search_public_dishes_by_tags_strict IS 'Search public dishes with strict tag filtering, returns all dish tags in matched_tags JSONB array';
